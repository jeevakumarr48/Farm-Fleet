...
export async function process(request: Request, response: Response) { if (!request.file) return sendError(response, 400, 'AUDIO_REQUIRED', 'Upload an audio file to process.'); const result = await voiceProcessor.transcribeAndExtract(request.file); response.status(201).json(result) }
